...
import { fetchDeposit as fetchDeposits, fetchDepositByOrder, approveDeposit, setAuthToken } from '@/lib/api';
...
import type { DepositResponse, DepositItem, DepositFilters } from '@/types/deposits';
...
const Deposits = () => {
  ...
  const loadByUserId = useCallback(async (p = 1) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.msg || 'Failed to fetch user deposits');
    } finally {
      setLoading(false);
    }
  }, [token, userId]);
  ...
  const loadByOrderId = useCallback(async () => {
    ...
    try {
      const response = await fetchDepositByOrder(q);
            ...
            <tbody>
              {data.items.map((d: DepositItem, i: number) => (
                <tr key={i} className="border-b border-border last:border-0 hover:bg-secondary/20 transition-colors">
                  ...
                  <td className="p-2 text-muted-foreground text-[10px] max-w-[120px] truncate" title={d.note}>{d.note || '-'}</td>
